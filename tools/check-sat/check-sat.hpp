...
public:
  ...
  void CardinalityConstraint(vector<int>& lits, int bound, double weight, string s_weight) override {
  ...
  */
  void checkSat () {
    ...
    for (auto tpl : cardinality_constraints) {
      int sat = 0;
  ...
  */
  ...
  void printResult () {
    ...
    if (has_weight ()) {
      ...
      if ( card_nunsat + nunsat == 0) cout << "c VERIFIED SATISFIABLE" << endl;
      else {
      ...
      if (cardUnsat+clauseUnsat == 0) cout << "c VERIFIED SATISFIABLE" << endl;
      else {
