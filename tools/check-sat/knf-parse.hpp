...
public:
  ...
  virtual void CardinalityConstraint(vector<int>& clause, int bound, double weight, string s_weight) {};
    ...
    while (c != EOF) {
      ...
      if (input_type != WCARD) {
        ...
        if(statement_type == 'c') { // clause
          ...
          ncls++;
          for(auto observer : observers_) {
            observer->CardinalityConstraint(clause, bound, weight, s_weight);
          }
        } 
